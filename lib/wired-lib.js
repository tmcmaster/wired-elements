...
export function tryUntil(action, condition, callback, timeout, interval) {
    let value = action();
    if (condition(value)) {
        callback(value);
    }
...
export function fireWindowEvent(name, detail, bubbles = true, composed = true) {
    fireEventTo(window, name, detail, bubbles, composed);
}
export function fireEventTo(destination, name, detail, bubbles = true, composed = true) {
    if (destination && name) {
        const init = {
...
export function delayCall(func, wait, immediate, context) {
    let timeout = 0;
    return () => {
